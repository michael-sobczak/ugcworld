...
## When true, left-clicking empty space pans the view instead of drag-box selecting.
@export var left_click_pans: bool = false
## Current zoom amount
...
func _handle_mouse_motion(event: InputEventMouseMotion):
    ...
    match event.button_mask:
        ...
        MOUSE_BUTTON_LEFT:
            ...
            elif left_click_pans:
                # Left-click on empty space pans the view
                _h_scroll_bar.value -= event.relative.x
                _v_scroll_bar.value -= event.relative.y
                queue_redraw()
