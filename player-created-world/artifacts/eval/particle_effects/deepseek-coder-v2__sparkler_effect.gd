...
func _ready() -> void:
    ...
    grad.colors = PackedColorArray([Color(1, 0.5, 0, 1), Color(1, 0.9, 0, 0)])
...
func _create_texture() -> ImageTexture:
    ...
    for y in range(16):
        for x in range(16):
            var d: float = Vector2(x, y).distance_to(Vector2(8, 8))
            var a: float = clampf(1.0 - d / 8.0, 0.0, 1.0)
            img.set_pixel(x, y, Color(1.0, 0.8, 0.0, a))
