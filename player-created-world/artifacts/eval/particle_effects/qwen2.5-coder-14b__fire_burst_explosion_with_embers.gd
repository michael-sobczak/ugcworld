...
func _ready() -> void:
    ...
    particles.amount = 64               # particle count
    particles.lifetime = 1.5            # seconds each particle lives
    particles.one_shot = true           # single burst, not looping
    particles.emitting = false          # starts inactive, play_at() triggers it
    particles.explosiveness = 1.0       # 0=stream, 1=all at once
    ...
    mat.scale_min = 0.1
    mat.scale_max = 0.3
    mat.angular_velocity_min = 100.0
    mat.angular_velocity_max = 200.0

    # Color — use a single color or a gradient ramp
    var grad := Gradient.new()
    grad.colors = PackedColorArray([Color(1, 0.5, 0, 1), Color(1, 0.5, 0, 0)])
...
func _create_texture() -> ImageTexture:
    ...
    for y in range(32):
        for x in range(32):
            ...
            img.set_pixel(x, y, Color(1.0, 0.5, 0, a))
